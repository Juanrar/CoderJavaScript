...
- [~] **Nombres descriptivos** (variables, funciones, archivos) → hecho: renombradas las funciones/variables que mentían (`agregarCarrito`→`agregarAlCarrito`, `eliminarCarrito`→`restarDelCarrito`, `traerDatos`→`traerJuegos`) y **resuelto el choque de los cuatro "contenedor"**: `seccionCatalogo` / `grillaJuegos` / `seccionCarrito` / `grillaCarrito` (15 apariciones, con F2 de VS Code). El nombre ahora dice el rol: `seccion*` es lo que se muestra/oculta, `grilla*` es donde se inyecta HTML. Los strings de los selectores (`"#contenedor-catalogo"`, `"#contenedor-carrito"`) **no** se tocaron: son del HTML y son independientes de los nombres JS. También renombrados: `numeroTotal`→`totalCarrito`, los dos `div` genéricos→`tarjetaJuego`/`itemCarrito`, `productoDescontado`→`juegoEnCatalogo` (+ su parámetro `productoStock`→`juego`), `contador`→`cantidadTotal`, `index`→`indice`. **Único pendiente (opcional):** el archivo habla dos idiomas para la misma cosa — vocabulario "juegos" (`juegos.json`, `traerJuegos`, `grillaJuegos`) vs "productos" (`productos`, `cargarProductos`, `productosCarrito`, `.producto` en CSS). Unificar a "juegos" son ~25 apariciones en JS **más** las clases del HTML/CSS; si no se hace, conviene mencionarlo en la reflexión de las Slides como algo detectado.
- ⚠️ **Renombrar SIEMPRE con F2 (rename simbólico) de VS Code, nunca con buscar-y-reemplazar de texto.** El reemplazo masivo rompió el archivo tres veces: (1) `-1`→`null` pisó las comparaciones de `findIndex` en `agregarAlCarrito` → `productosCarrito[-1].cantidad` → `TypeError` en el primer "Agregar" de cualquier usuario; (2) al borrar las declaraciones globales de los botones quedaron **globales implícitas** (asignación sin `const` → propiedad de `window`, y `ReferenceError` si algún día se usa `"use strict"` o módulos ES); (3) `index`→`indice` convirtió `findIndex` en `findindice` → `TypeError`, con agregar y restar del carrito muertos. F2 entiende qué es un símbolo y qué es texto; buscar-y-reemplazar no.
- [ ] **Comentarios breves** donde aporten claridad → criterio acordado: comentar el **por qué**, nunca el qué; entre 4 y 6 comentarios en todo el archivo
- [x] **Sin** código comentado / en desuso → **hecho.** Se eliminaron: el `setItem("carrito", …)` suelto de `finalizarCompra` (dejaba la clave en un tipo distinto y solo "funcionaba" porque la línea siguiente lo pisaba), las tres globales `botonesAgregar`/`botonesCarritoSuma`/`botonesCarritoResta` (nadie leía su valor inicial — y al borrarlas quedaron globales **implícitas** hasta agregarles el `const` dentro de cada función), y las tres copias de `new Intl.NumberFormat(...)` → extraídas a `formatearPrecio(precio)`
- [x] Estructura y modularización claras para la corrección → `formatearPrecio` y `mostrarCatalogo` extraídas; cada función de render limpia solo su grilla; `guardarCarrito` como única dueña del `localStorage`
- [~] Pasada final de revisión completa → **hecha la revisión; 5 hallazgos identificados y POSPUESTOS a conciencia por falta de tiempo antes de la entrega.** Ninguno afecta el 45 % (flujo) ni el 35 % (datos/librerías): **(1)** `id` duplicados en el DOM — el `.boton-agregar` del catálogo y los `.boton-sumar`/`.boton-restar` del carrito usan `id="${producto.id}"`, así que con un juego en el carrito hay tres elementos con el mismo `id` (HTML inválido). Arreglo: pasar a `data-id` + `e.currentTarget.dataset.id`, coherente con el `data-categoria` que ya se usa. Riesgoso a último momento porque toca 4 líneas del flujo principal. **(2)** El `reduce` del listener de `.boton-finalizar` solo sirve para saber si el carrito está vacío → `productosCarrito.length === 0` alcanza (ningún ítem puede tener `cantidad: 0`, `restarDelCarrito` hace `splice` al llegar a 0). **(3)** `traerJuegos`: `const datos` existe solo para asignarse a `productos` en la línea siguiente; opcional, chequear `respuesta.ok` (`fetch` no lanza en 404/500). **(4)** El `if` de `mostrarCatalogo` es innecesario (`classList.add`/`remove` son idempotentes). **(5)** Cosmético: resuelto con Shift+Alt+F. **Los hallazgos 1 y el vocabulario `productos`/`juegos` se mencionan en la reflexión de las Slides** como decisiones conscientes de priorización.
